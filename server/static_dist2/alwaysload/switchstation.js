...
class Route {
    ...
    load(urlm, views_attach_point) {
        return new Promise(async (res, _rej)=>{
            ...
            const els = parentEl.querySelectorAll(`v-${viewname}`);
...
function Back() {
    const backhash = document.querySelector("#views .view[active]").getAttribute("backhash");
    window.location.hash = backhash;
}
function load_and_attach_route(url, views_attach_point) {
    return new Promise(async (res, _rej)=>{
        const [urlm, route] = load_and_attach_route___set_match_and_get_match_and_route(url);
        route.load(urlm, views_attach_point).then(()=>{
            res(1);
        });
    });
}
function load_and_attach_route___set_match_and_get_match_and_route(url) {
    ...
    return [
        "home".match(/home/g).slice(1),
        _routes.find((r)=>r.lazyload_view.name === "home")
    ];
}
async function hash_changed(e = null) {
    const overlayel = document.getElementById("switchstation_overlay");
    ...
    if (document.querySelector("#views .view[active][draggedback]")) {
        const activeview = document.querySelector("#views .view[active][draggedback]");
        const previousview = activeview.previousElementSibling;
        activeview.remove();
        ...
        const view = document.querySelector("#views .view");
        ...
        if (!previousview) {
            ...
            previousview = activeview?.previousElementSibling;
        ...
        function activeview_transitionend() {
            activeview?.remove();
            previousview?.setAttribute("active", "");
            const previous_previousview = previousview?.previousElementSibling;
        ...
        await load_and_attach_route(window.location.hash.substring(1), "beforeend");
        const allviews = document.querySelectorAll("#views .view");
