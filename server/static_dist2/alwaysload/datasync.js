...
function Init(indexeddb_stores, dbname, dbversion, appversion) {
    worker = new Worker("/assets/lazy/workers/datasync_worker.js", {
        type: "module"
    });
    worker.postMessage({
        cmd: "init",
    ...
    const store_metas = JSON.parse(localStorage.getItem("datasync_store_metas") || "[]");
    ...
    SSEvents.Add_Listener("datasync", [
        SSE_TriggersE.FIRESTORE
    ], (data)=>{
        const store_metas = JSON.parse(localStorage.getItem("datasync_store_metas") || "[]");
        const filtered_store_metas = store_metas.filter((s)=>data.paths.includes(s.n));
        for (const s of filtered_store_metas){
            s.l = 2 /* STAlE*/ ;
        }
        localStorage.setItem("datasync_store_metas", JSON.stringify(store_metas));
        worker?.postMessage({
...
function Subscribe(store_names, subscriber_el) {
    const store_metas = JSON.parse(localStorage.getItem("datasync_store_metas") || "[]");
    worker?.postMessage({
        cmd: "subscribe",
        store_names,
        subscriber: generate_string_from_el(subscriber_el),
        store_metas
    });
}
function notify_subscribers(subscribers) {
...
function save_store_metas(store_metas) {
    for (const s of store_metas){
        for (const sm of s.s){
...
function generate_string_from_el(el) {
    let path_str = "";
    let current_element = el;
...
function get_el_from_subscriber_str(el_str) {
    const el_str_parts = el_str.split(",");
    let current_element = document.querySelector(el_str_parts[0]);
    for(let i = 1; i < el_str_parts.length; i++){
        if (current_element) {
            current_element = current_element.shadowRoot?.querySelector(el_str_parts[i]);
        }
    }
    return current_element;
}
function remove_store_metas_not_in_indexeddb_store_metas(indexeddb_stores, store_metas) {
    for (const s of store_metas){
