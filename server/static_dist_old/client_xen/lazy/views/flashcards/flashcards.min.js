...
`;var VFlashCards=class extends HTMLElement{s;flashcards;shadow;constructor(){super();this.s={props:{}};this.flashcards=[];this.shadow=this.attachShadow({mode:"open"});SetDistCSS(this.shadow,distcss)}async connectedCallback(){const results=await Firestore.Retrieve("machines");this.flashcards=results[0];this.sc();this.dispatchEvent(new Event("hydrated"))}sc(state_changes={}){this.s=Object.assign(this.s,state_changes);Lit_Render(this.template(this.s,this.flashcards),this.shadow)}template=(_s,_flashcards)=>{return Lit_Html`
