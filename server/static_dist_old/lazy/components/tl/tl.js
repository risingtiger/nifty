(() => {
  // server/static_dev/lazy/components/tl/tl.js
  var CTl = class extends HTMLElement {
    s;
    ...
    async connectedCallback() {
      ...
      if (customelements_to_load.length > 0)
        await LazyLoad(customelements_to_load.map((name) => {
          return {
            what: "components",
            name
          };
  ...
  function find_all_custom_components_to_load(container_element) {
    const allels = [];
    const allels_str = [];
    for (const el of container_element.children) {
      if (el.tagName.startsWith("C-"))
        allels.push(el);
      for (const ell of el.children) {
        if (ell.tagName.startsWith("C-"))
          allels.push(ell);
      }
    }
