...
import * as SSEvents from "./sse.js"
...
function Init(collection_names:str[]) {
    ...
    collection_names.forEach(cn=> {
        const modts = Number(localStorage.getItem("modts_"+cn) || "0")
        fcollections.push({
            name: cn,
            status: modts === 0 ? CollectionStatus.Empty : CollectionStatus.Stale,
            modts,
            opt: { propa: "" }
        })
    })

    AppFocusListen.AppFocus_Add_Listener("firestore", (isfocused:bool)=> {
    ...
    SSEvents.Add_Listener("firestore", SSEvents.SSE_Triggers.FIRESTORE, (paths:str[])=> {
...
async function Subscribe(htmlel:HTMLElement, p_resources:str[]|str) {
    ...
    return new Promise(async (res, _rej)=> {
        ...
        if (is_already_listener) {
            redirect_from_error("Firestore Listener with that name already exists")
            return
        }
        ...
        if (collections_to_update.length) {
            ...
            const fetch_results = await fetch_collections_since_modts(collections_to_update)
...
function fetch_collections_since_modts(collections:Collection[]) {
    ...
    return new Promise<{collection_name:str,docs_array:object[]}[]>(async (res)=> {
        ...
        const body = { 
            paths:collections.map(p=> p.name),
            opts:collections.map(p=> {
                return  {order_by:null, limit:null, modts: p.modts} 
            }) 
        }
        ...
        const f = await FetchLassie('/api/firestore_retrieve', opts) as Promise<Array<Array<object>>>
    ...
    return new Promise(async (res)=> {
        ...
        tx.oncomplete = (_event) => {
            ...
            collections.forEach(c=> {
                ...
                c.modts = Math.floor(Date.now()/1000)
                localStorage.setItem("modts_"+c.name, c.modts.toString())
            })
