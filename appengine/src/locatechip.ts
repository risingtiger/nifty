...
const LocateParticleChipByCellTower = (_db:any, particleAccount:str, particleId:str, secretsClient:any) => {
  ...
  return new Promise(async (res, _rej)=> {
    ...
    const diagnostics = await diagnosticsP.json() as any
    ...
    if (cellular && cellGlobalIdentity) {
      ...
      const googlAPIResponseP = await fetch(`https://www.googleapis.com/geolocation/v1/geolocate?key=AIzaSyCdBd4FDBCZbL03_M4k2mLPaIdkUo32giI`, 
           {   method: "POST", headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(googleapiObjToSend)   })
